                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-4 py-3 text-xs text-gray-500">
                            {{ $pago->contrato->propiedad->tipo_label }}<br>
                            {{ $pago->contrato->propiedad->ciudad }}
                        </td>
        ...
        <div class="bg-white rounded-xl shadow-xl w-full max-w-lg max-h-screen overflow-y-auto">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 sticky top-0 bg-white">
                <h3 class="font-semibold text-gray-800">
                    {{ $pagoId ? 'Registrar cobro de cuota' : 'Nuevo Pago Manual' }}
                </h3>
                @if ($pagoId)
            ...
            <div class="p-6 space-y-4">
                <div>
                    ...
                    <select wire:model.live="contratoId" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 @error('contratoId') border-red-400 @enderror">
                        <option value="">Seleccionar contrato...</option>
                        @foreach($contratos as $c)
                            <option value="{{ $c->id }}">
                                {{ $c->inquilino->nombre_completo }} — {{ $c->propiedad->tipo_label }} {{ $c->propiedad->ciudad }}
                ...
                <div class="grid grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <input type="number" wire:model="descuento" step="0.01" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                </div>
