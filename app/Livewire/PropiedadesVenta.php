...
class PropiedadesVenta extends Component
{
    ...
    protected function rules(): array
    {
        return [
            ...
            'estado'          => 'required|in:disponible,vendida,reservada',
    ...
    protected $messages = [
        ...
        'precioVenta.numeric'  => 'El precio debe ser un número.',
        'precioVenta.min'      => 'El precio debe ser mayor a cero.',
    ...
    {
        $query = PropiedadVenta::with('propietario')
            ...
            ->when($this->filtroMoneda,  fn($q) => $q->where('moneda', $this->filtroMoneda))
            ->when($this->filtroPropietario, fn($q) => $q->where('propietario_id', $this->filtroPropietario))
            ->orderByDesc('id');

        return view('livewire.propiedades-venta', [
    ...
    {
        $p = PropiedadVenta::findOrFail($id);
        $this->propiedadId       = $id;
        ...
        $this->precioVenta       = $p->precio_venta;
    ...
    public function guardar(): void
    {
        ...
        foreach ($this->nuevasFotos as $foto) {
            $fotos[] = $foto->store('propiedades-venta', 'public');
        }
        ...
        $datos = [
            ...
            'precio_venta'       => (float) $this->precioVenta,
            'moneda'             => $this->moneda,
        ...
        if ($this->propiedadId) {
            PropiedadVenta::findOrFail($this->propiedadId)->update($datos);
    ...
    {
        PropiedadVenta::findOrFail($id)->update(['estado' => $nuevoEstado]);
